---
layout: page
title: Projects
permalink: /projects/
---

<div class="projects-container">
  <div class="project-item">
    <div class="project-info">
      <h2 class="project-title">rustgrad</h2>
      <p class="project-description">Micrograd-inspired, tiny autograd engine and neural network library with PyTorch-like API, written in Rust 🦀</p>
      <div class="project-meta">
        <span class="project-language rust">Rust</span>
        <a href="https://github.com/hltk/rustgrad" target="_blank" class="project-link">View on GitHub →</a>
      </div>
    </div>
    <div class="project-image">
      <svg viewBox="0 0 400 300" xmlns="http://www.w3.org/2000/svg">
        <rect width="400" height="300" fill="#f0f0f0"/>
        <text x="50%" y="50%" text-anchor="middle" fill="#999" font-size="24" font-family="sans-serif">rustgrad</text>
      </svg>
    </div>
  </div>

  <div class="project-item">
    <div class="project-info">
      <h2 class="project-title">gitgo</h2>
      <p class="project-description">Web interface for Git repositories written in Go 🐳</p>
      <div class="project-meta">
        <span class="project-language go">Go</span>
        <a href="https://github.com/hltk/gitgo" target="_blank" class="project-link">View on GitHub →</a>
      </div>
    </div>
    <div class="project-image">
      <svg viewBox="0 0 400 300" xmlns="http://www.w3.org/2000/svg">
        <rect width="400" height="300" fill="#f0f0f0"/>
        <text x="50%" y="50%" text-anchor="middle" fill="#999" font-size="24" font-family="sans-serif">gitgo</text>
      </svg>
    </div>
  </div>

  <div class="project-item">
    <div class="project-info">
      <h2 class="project-title">contestlib</h2>
      <p class="project-description">Data structures and algorithms for ICPC-style programming contests 🤖</p>
      <div class="project-meta">
        <span class="project-language cpp">C++</span>
        <a href="https://github.com/hltk/contestlib" target="_blank" class="project-link">View on GitHub →</a>
      </div>
    </div>
    <div class="project-image">
      <svg viewBox="0 0 400 300" xmlns="http://www.w3.org/2000/svg">
        <rect width="400" height="300" fill="#f0f0f0"/>
        <text x="50%" y="50%" text-anchor="middle" fill="#999" font-size="24" font-family="sans-serif">contestlib</text>
      </svg>
    </div>
  </div>

  <div class="project-item">
    <div class="project-info">
      <h2 class="project-title">adventofcode</h2>
      <p class="project-description">Advent of Code problems solved in various languages 🎄</p>
      <div class="project-meta">
        <span class="project-language python">Python</span>
        <a href="https://github.com/hltk/adventofcode" target="_blank" class="project-link">View on GitHub →</a>
      </div>
    </div>
    <div class="project-image">
      <svg viewBox="0 0 400 300" xmlns="http://www.w3.org/2000/svg">
        <rect width="400" height="300" fill="#f0f0f0"/>
        <text x="50%" y="50%" text-anchor="middle" fill="#999" font-size="24" font-family="sans-serif">adventofcode</text>
      </svg>
    </div>
  </div>
</div>
